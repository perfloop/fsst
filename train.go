...
const (
	...
	// Hold one retained prefix and one incoming batch in reusable selection
	// scratch while ranking oversized candidate maps.
...
func Train(inputs [][]byte) *Table {
	...
	for frac := 8; ; frac += 30 {
		...
		buildCandidates(table, &workspace.counter, frac, workspace.candidates, &workspace.scratch, &workspace.list)
		if frac >= 128 {
			break
		}
	}
...
func newTrainingWorkspace() *trainingWorkspace {
	return &trainingWorkspace{
		...
		scratch:    make(qsymHeap, 0, maxCandidateScratchSymbols),
	}
}
...
// selectStrongestCandidates partitions candidates around the retained prefix.
...
//  3. Keep the strongest candidates in bounded scratch, including enough
//     extras to backfill symbols rejected by hash-table collisions. Partition
//     oversized batches, then sort only the retained prefix by total rank.
//
// The map and selection scratch are reused across iterations to reduce GC pressure.
func buildCandidates(t *Table, c *counters, frac int, candidates map[[2]uint64]qsym, scratch *qsymHeap, list *[]qsym) {
	...
	selectCandidates(candidates, scratch, list)
...
func selectCandidates(candidates map[[2]uint64]qsym, scratch *qsymHeap, list *[]qsym) {
	selected := (*scratch)[:0]
